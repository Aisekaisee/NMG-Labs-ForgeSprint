...
def seo_fix() -> dict:
    fix_rewriter.OLLAMA_CALLS_COUNT = 0
    rows = RUN.get("rows", [])
    issues = RUN.get("issues", [])
    
    titles = fix_rewriter.rewrite_titles(rows, issues, model=MODEL)
    metas = fix_rewriter.rewrite_metas(rows, issues, model=MODEL)
    redirect_map = fix_rewriter.build_redirect_map(rows, issues)
    ...
    RUN["model_calls"] = fix_rewriter.OLLAMA_CALLS_COUNT
...
def _run_mcp():
    ...
    @mcp.tool()
    def fix_issues() -> dict:
        """Run all three fixer functions (rewriting titles, metas, and redirect map) via Ollama."""
        return seo_fix()
